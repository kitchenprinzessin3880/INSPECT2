...
public class ServiceMetadataServiceImpl implements ServiceMetadataService {
    ...
    public GetProcedureResponse getProcedure(String serviceURL, String procedureID) throws Exception {
        try {
            ...
            ArrayList<Offering> o = meta.getOfferings();
    ...
    //ASD
	@Override
	public GetOfferingResponse getAllOffering(String serviceURL) throws Exception {
		// TODO Auto-generated method stub
		try {
            if (LOG.isDebugEnabled()) {
                String msgTemplate = "Request -> getAllOfferingBySOS(serviceURL: %s)";
                LOG.debug(String.format(msgTemplate, serviceURL));
            }
            SOSMetadata meta = ConfigurationContext.getSOSMetadata(serviceURL);
            ArrayList<String> offSet = meta.getOfferingsCustom();

            return new GetOfferingResponse(serviceURL, offSet);
