...
public class RpcStationPositionsServlet extends RemoteServiceServlet implements
		RpcStationPositionsService {

	private static final long serialVersionUID = -7915385111187214262L;

	private static final Logger LOG = LoggerFactory
			.getLogger(RpcStationPositionsServlet.class);
	...
	public void init() throws ServletException {
		LOG.debug("Initialize " + getClass().getName()
				+ " Servlet for SOS Client");
		service = new StationPositionsServiceImpl();
	}

	public StationPositionsResponse getStationPositions(String sosURL,
			int start, int interval, BoundingBox boundingBox) throws Exception {
		Statistics
				.saveHostRequest(this.getThreadLocalRequest().getRemoteHost());
		return service
				.getStationPositions(sosURL, start, interval, boundingBox);
	}
}