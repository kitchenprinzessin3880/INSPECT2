...
public class RpcEESDataServlet extends RemoteServiceServlet implements RpcEESDataService {
	...
	public EESDataResponse getDiagrams(EESDataRequest request) throws Exception {
		// TODO Auto-generated method stub
		Statistics.saveHostRequest(this.getThreadLocalRequest().getRemoteHost());
		EESDataResponse response =null;
		try
		{
			 response = service.getDiagrams(request);
		}
		// Exception thrown when network timeout occurs
		catch (InterruptedIOException iioe)
		{
			LOG.debug("Remote host timed out during read operation");
		}
		// Exception thrown when general network I/O error occurs
		catch (IOException ioe)
		{
			LOG.debug("Network I/O error - " + ioe);
		}
        return response;
	}
}
