...
/**
 * A shared metadata representation for an SOS instance. An {@link SOSMetadata}
 * is used from both (!) Client side and Server side. Depending on if the SOS
 * metadata representation is used on either Client sider or Server side,
 * attributes have to be set differently (see constructor notes) ! It is the
 * developer's responsibility to keep them in sync.
 * 
...
public class SOSMetadata implements Serializable {
	...
	private int requestChunk = 400; // default
	...
	/**
	 * Use this constructor only for non-configurated SOS instances! Prefer
	 * using {@link SOSMetadata#SOSMetadata(SOSMetadataBuilder)}.
	 * 
	 * @param url
	 *            the service URL
	 * @param title
	...
	public void removeProcedure(String ID) {
		Set<Entry<String, Station>> entrySet = this.stations.entrySet();
	...
	//ASD
	public ArrayList<String> getOfferingsCustom() {
		ArrayList<String> offs = new ArrayList<String>(this.offerings.keySet());
		return offs;
	}
	...
	/**
	 * @return the service's extent or the {@link Constants#FALLBACK_EXTENT} if
	 ...
	 * @return the station at given {@link EastingNorthing} location, or
	 *         <code>null</code> if no station is available.
	 */
	...
	public Station getStation(String offeringId, String featureId, String procedureId, String phenomenonId) {
		for (Station station : stations.values()) {
			if (station.isOfferingEqual(offeringId) && station.isFeatureEqual(featureId) && station.isProcedureEqual(procedureId) && station.isPhenomenonEqual(phenomenonId)) {
				return station;
			}
		}
		return null;
	...
	// ASD
	public void removeProcedureById(String pId) {
		Procedure p = this.procedures.get(pId);
		if (p != null) {
			this.procedures.remove(pId);
		} else {
	...
	// ASD
	public void removeFOIById(String fId) {
		FeatureOfInterest f = this.features.get(fId);
		if (f != null) {
