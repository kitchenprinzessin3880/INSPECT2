...
public class DiagramGenerator extends Generator {
	 ...
	 * @throws IOException
	 */
	public void producePresentation(Map<String, OXFFeatureCollection> entireCollMap, DesignOptions options, FileOutputStream out, boolean compress) throws OXFException, IOException {
		...
		// ASD
		DiagramRenderer renderer = new DiagramRenderer(false, true);
		...
		JFreeChart diagramChart = null;
		// ASD commented out 8.5.2014
		diagramChart = renderer.renderChart(entireCollMap, options, begin, end, compress);
	...
	public void producePresentationCustom(Map<String, TimeSeriesCollection> mainDatasetMap, DesignOptions options, FileOutputStream out, boolean compress) throws OXFException, IOException {
		...
		// ASD
		TimeSeriesRenderer renderer = new TimeSeriesRenderer(true, this, mainDatasetMap);

		JFreeChart diagramChart = null;
		// ASD commented out 8.5.2014
		diagramChart = renderer.renderChart(options, begin, end, compress);
		diagramChart.removeLegend();
		// draw chart into image:
		BufferedImage diagramImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
		Graphics2D chartGraphics = diagramImage.createGraphics();
	 ...
	 *             the oXF exception
	 */
	public void produceLegend(DesignOptions options, OutputStream out) throws OXFException, IOException {
		...
		for (int i = 0; i < ddList.size(); i++) {
			...
			/*
			 * legendGraphics.drawString(dd.getFeatureOfInterestDesc() + " - "
			 * //$NON-NLS-1$ + dd.getLabel(), leftMargin + iconWidth +
			 * horizontalSpaceBetweenIconAndText, yPos + iconHeight);
			 */
			// ASD
			legendGraphics.drawString(dd.getFeatureOfInterestDesc() + " - " //$NON-NLS-1$
					+ dd.getLabel() + " - " + dd.getObservedPropertyID(), leftMargin + iconWidth + horizontalSpaceBetweenIconAndText, yPos + iconHeight);
		}
	...
	private DesignDescriptionList buildUpDesignDescriptionList(DesignOptions options) {
		...
		for (TimeSeriesProperties prop : options.getProperties()) {
			...
			ddList.add(prop.getPhenomenon().getId(), prop.getProcedure().getId(), prop.getFoi().getId(), prop.getPhenomenon().getLabel(), prop.getProcedure().getLabel(), prop.getFoi().getLabel(), prop.getLabel(), prop.getUnitOfMeasure(), c, prop.getGraphStyle(), prop.getLineWidth(), gridOn);
		}
