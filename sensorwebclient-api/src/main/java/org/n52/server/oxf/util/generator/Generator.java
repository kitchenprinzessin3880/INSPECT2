...
public abstract class Generator {
    ...
    protected String createAndSaveImage(DesignOptions options, JFreeChart chart, ChartRenderingInfo renderingInfo, boolean isOverview) throws GeneratorException {
        int width = options.getWidth();
        int height = options.getHeight();
        //Size2D size = chart.getLegend().arrange((Graphics2D) bfi.getGraphics(),new RectangleConstraint(width, height));
        //ASD
        BufferedImage image =null;
        Graphics2D chartGraphics = null;
        if (!isOverview)
        {
            LegendTitle legend=new LegendTitle(chart.getPlot());
            int someHeight = (int) legend.getHeight();
        	image = chart.createBufferedImage(width, height+someHeight, renderingInfo);
            chartGraphics = image.createGraphics();
            chartGraphics.setColor(Color.white);
            chartGraphics.fillRect(0, 0, width, height);
            chart.draw(chartGraphics, new Rectangle2D.Float(0, 0, width, height+someHeight));
        }
        else
        {
