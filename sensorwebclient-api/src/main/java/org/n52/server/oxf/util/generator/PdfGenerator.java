...
public class PdfGenerator extends Generator {
	...
	Map<String, TimeSeriesCollection> mainDatasetMap = new HashMap<String, TimeSeriesCollection>();
	 ...
	 *             the exception
	 */
	private DocumentStructureDocument buildUpDocumentStructure(DesignOptions getRepresentationOp) throws Exception {
		...
		for (TimeSeriesProperties prop : getRepresentationOp.getProperties()) {
			...
			// observationCollMap.get(offering.getId() + "@" +
			// prop.getSosUrl());

			org.n52.oxf.DocumentStructureType.TimeSeries timeSeries = docStructure.addNewTimeSeries();
			...
			// timeSeries.setTable(buildUpTable(prop, obsColl, foiID,
			// obsPropsID, procID));
		}
	...
	private MetadataType buildUpMetadata(String sosURL, String procedureID) throws Exception {
		...
		XmlObject xmlObject = XmlObject.Factory.parse(opResult.getIncomingResultAsStream(), xmlOpts);
	 ...
	 */
	private TableType buildUpTable(TimeSeriesProperties prop, OXFFeatureCollection obsColl, String foiID, String obsPropID, String procID) {
		...
		table.setRightColHeader(prop.getPhenomenon().getLabel() + " (" + prop.getPhenomenon().getUnitOfMeasure() + ")");
		...
		ObservationSeriesCollection seriesCollection = new ObservationSeriesCollection(obsColl, new String[] { foiID }, new String[] { obsPropID }, new String[] { procID }, true);
	...
	public RepresentationResponse producePresentation(DesignOptions options) throws GeneratorException {
		...
		try {
			...
			DocumentStructureDocument docStructureDoc = buildUpDocumentStructure(options);
			...
			// produce chart image:
			File imageFile = JavaHelper.genRndFile(ConfigurationContext.GEN_DIR, "chartImage_", DiagramGenerator.FORMAT);
			File legendFile = JavaHelper.genRndFile(ConfigurationContext.GEN_DIR, "legendImage_", DiagramGenerator.FORMAT);
			...
			// cocoonXslDir:
			File docStructureFile = JavaHelper.genRndFile(ConfigurationContext.GEN_DIR, "pdfDoc_", "xml");
			...
			if (this.zip) {
				...
				this.pdfFile = JavaHelper.genRndFile(ConfigurationContext.GEN_DIR + "/" + folderPostfix, pc.getProcedure().getId().replaceAll("/", "_") + "_" + formatDate(new Date(options.getBegin())) + "_" + formatDate(new Date(options.getEnd())) + "_", "pdf");
				this.pdfURL = ConfigurationContext.GEN_URL + this.pdfFile.getName();
			} else {
				if (options.getProperties().size() > 1) {
					this.pdfFile = JavaHelper.genRndFile(ConfigurationContext.GEN_DIR + "/" + folderPostfix, "Cumulated_PDF_", "pdf");
					this.pdfURL = ConfigurationContext.GEN_URL + folderPostfix + "/" + this.pdfFile.getName();
				} else {
					TimeSeriesProperties pc = options.getProperties().get(0);
					this.pdfFile = JavaHelper.genRndFile(ConfigurationContext.GEN_DIR + "/" + folderPostfix, pc.getProcedure().getId().replaceAll("/", "_") + formatDate(new Date(options.getBegin())) + "_" + formatDate(new Date(options.getEnd())) + "_", "pdf");
					this.pdfURL = ConfigurationContext.GEN_URL + folderPostfix + "/" + this.pdfFile.getName();
				}
	...
	public RepresentationResponse producePresentationCustom(DesignOptions options) throws GeneratorException {
		// Map<String, OXFFeatureCollection> observationCollMap =
		// getFeatureCollectionFor(options, false);
		try {
			// produce document structure:
			DocumentStructureDocument docStructureDoc = buildUpDocumentStructure(options);
			...
			for (int i = 0; i < options.getProperties().size(); i++) {
				...
				TreeMap<Date, DataValue> dataValue = ObservationData.getInstance().getLocalObservationData().get(timSeriesId);
				
				Log.debug("XXXXXXXXXXXXXX size: " + dataValue.size());
				Iterator it = dataValue.entrySet().iterator();
				while (it.hasNext()) {
					Map.Entry pairs = (Map.Entry) it.next();
					DataValue data = (DataValue) pairs.getValue();
					Date timeStmp = data.getTimestampto();
					Double resultVal = data.getDatavalue();
					mainTimeSeries.add(new Second(timeStmp), resultVal);
				}
			...
			try {
				...
				diagramGen.producePresentationCustom(mainDatasetMap, options, imageOut, false);
				LOGGER.debug("imageFile: " + imageFile);
