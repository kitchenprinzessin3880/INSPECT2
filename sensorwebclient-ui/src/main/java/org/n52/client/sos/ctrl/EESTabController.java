...
public class EESTabController extends Controller<EESTab> {
	...
	private class EESTabEventBroker extends ATabEventBroker implements TabSelectedEventHandler, ResizeEventHandler, TimeSeriesChangedEventHandler, RequestDataEventHandler, NewTimeSeriesEventHandler, InitEventHandler, DeleteTimeSeriesEventHandler, SetImageUrlEventHandler, SwitchAutoscaleEventHandler, ChangeImagePixelBoundsEventHandler,
			DatesChangedEventHandler, SwitchGridEventHandler, StoreTimeSeriesPropsEventHandler, DeleteAllTimeSeriesEventHandler {
		...
		public void onTimeSeriesChanged(TimeSeriesChangedEvent evt) {
			...
			if (DataStoreTimeSeriesImpl.getInst().getDataItems().isEmpty()) {
				...
				// Commented out by ASD @ 10.05.2014 - multiple events
				// generated,
				// e.g., (OverviewPresenter)
				// onSetDomainBounds,onSetOverviewDomainBounds due to a new
				// slider creation.
		...
		public void onDeleteTimeSeries(DeleteTimeSeriesEvent evt) {
			...
			if (DataStoreTimeSeriesImpl.getInst().getDataItems().isEmpty()) {
				EESTabController.this.tab.hideTooltips();
				EESTab.getRightInteractionMenuButton().setDisabled(true);
				EESTab.getLeftInteractionMenuButton().setDisabled(true);
				EESTab.getCenterInteractionMenuButton().setDisabled(true);
			}
		}
		...
		public void onNewTimeSeries(NewTimeSeriesEvent evt) {
			getTab().showLoadingSpinner();
			if (isSelfSelectedTab()) {
				EESTabController.this.getTab().addSlider(); // TODO check, if
															// onUpdate is
															// enough
				/*
		 ...
		 * 
		 * @see
		 * org.n52.client.eventBus.events.handler.InitEventHandler#onInit(org
		 * .n52.client.eventBus.events.InitEvent)
		 */
