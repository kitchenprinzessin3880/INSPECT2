...
public class SOSController extends ServiceController {
	...
	private static boolean qualityControlStatus = false;

	public SOSController() {
		new SosControllerEventBroker();
	}

	public SOSRequestManager getRequestManager() {
		return SOSRequestManager.getInstance();
	}

	private class SosControllerEventBroker implements NewTimeSeriesEventHandler, LoadImageDataEventHandler, RequestSensorDataEventHandler, GetPhenomenonsEventHandler, GetStationsEventHandler, GetProcedureDetailsUrlEventHandler, GetProcedureEventHandler, StoreProcedureEventHandler, GetOfferingEventHandler, StoreOfferingEventHandler,
			GetFeatureEventHandler, StoreFeatureEventHandler, GetStationEventHandler, StoreStationEventHandler, ExportEventHandler, FinishedLoadingTimeSeriesEventHandler, SetOverviewDomainBoundsEventHandler, SetDomainBoundsEventHandler, GetQualityCodeEventHandler, SetQualityControlStatusEventHandler,ResizeEventHandler {
		...
		public SosControllerEventBroker() {
			...
			EventBus.getMainEventBus().addHandler(GetQualityCodeEvent.TYPE, this);
		...
		public void onNewTimeSeries(NewTimeSeriesEvent evt) {
			try {
				...
				if (evt.getBuilderList() != null) {
					getRequestManager().requestMultipleSensorMetadata(evt);
				} else {
					getRequestManager().requestSensorMetadata(evt);
				}
			} catch (Exception e1) {
				GWT.log("SosController - onNewTimeSeries is failed.", e1);
			}
		}

		public void onLoadImageData(LoadImageDataEvent event) {
			// ASD
			if (!getRequestManager().isRequestDiagramRunning() && !QualityFlaggingController.getInstance().isAddFlagRunning()) {
				if (qualityControlStatus == true && QualityFlaggingController.getInstance().getRenderingPoints().size() > 0) {
					getRequestManager().requestDiagrams(true);
				} else {
					getRequestManager().requestDiagrams(false);
				}
			} else {
				Toaster.getInstance().addMessage("Please wait! Previous request is still on progress");
			}
		}
		...
		public void onGetProcedurePositions(GetStationsEvent evt) {
			getRequestManager().requestProcedurePositions(evt.getSOSURL(), evt.getBBox(), evt.isSearchByTheme());
		}
		...
		public void onGetOffering(GetOfferingEvent evt) {
			if (evt.isAllOfferingBySOS()) {
				getRequestManager().requestAllOffering(evt.getServiceURL(), evt.getOfferingID());
			} else {
		...
		public void onSetQualityControlStatus(SetQualityControlStatusEvent evt) {
			...
			qualityControlStatus = evt.getQCStatus();
		}

		@Override
		public void onGetQualityCodeEvent(GetQualityCodeEvent evt) {
			// TODO Auto-generated method stub
			getRequestManager().requestQualityCode();
		}
