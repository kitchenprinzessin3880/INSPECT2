...
public class Toaster {
	...
	public void addMessage(String msg) {
		...
		this.messages.clear();
		this.messages.add(l);
		int memberCount = this.layout.getMembers().length;
		for (int i = 0; i < memberCount; i++) {
			this.layout.removeMember(this.layout.getMember(i));
		}
		 ...
		 * this.messages.add(l);
		 * 
		 * for (int i = 0; i < this.messages.size(); i++) { if
		 * (this.layout.hasMember(this.messages.get(i))) {
		 * this.layout.removeMember(this.messages.get(i)); } } for (int i =
		 * this.messages.size() - 1; i >= 0; i--) {
		 * this.layout.addMember(this.messages.get(i)); }
		 */

		// this.left =
		// this.toasterWindow.getParentElement().getWidth().intValue() -
		// this.width - 10;
		// this.top =
		// this.toasterWindow.getParentElement().getHeight().intValue() -
		// this.height - 30;
		//
		// this.toasterWindow.setLeft(this.left);
		// this.toasterWindow.setTop(this.top);
	...
	public void addErrorMessage(String error) {
		 ...
		 * GWT.log(this.messages.get(i).toString());
		 * 
		 * } this.messages.add(l); for (int i = this.messages.size() - 1; i >=
		 * 0; i--) { this.layout.addMember(this.messages.get(i)); }
		 */
	...
	public void show() {
		...
		Toaster.this.toasterWindow.show();
		this.isVisible = true;
	}
