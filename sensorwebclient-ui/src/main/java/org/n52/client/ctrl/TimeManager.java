...
public class TimeManager {
    ...
    //ASD
    protected long intervalBeginFixed;
	protected long intervalEndFixed;
	protected boolean isOverviewTimeFixed = false;
    ...
    private class TimeManagerEventBroker implements
        ...
        public void onDatesChanged(DatesChangedEvent evt) {
            ...
            //ASD 
           	//long ovBegin = TimeManager.getInst().getIntervalBeginFixed();
			//long ovEnd = TimeManager.getInst().getIntervalEndFixed();
			
        	if (TimeManager.this.overviewInterval < (evt.getEnd() - evt.getStart()) * MIN_OVERVIEW_INTERVAL) {
				TimeManager.this.overviewInterval = (evt.getEnd() - evt.getStart()) * MIN_OVERVIEW_INTERVAL;
				setIsOverviewTimeFixed(false);
        	}
        	
            //ASD - disable the following:
         /*   TimeManager.this.begin = evt.getStart();
            	TimeManager.this.end = evt.getEnd();
            if (TimeManager.this.overviewInterval > (evt.getEnd() - evt.getStart()) * MAX_OVERVIEW_INTERVAL) {
				...
				GWT.log("MAX_OVERVIEW_INTERVAL:"+overviewInterval);
			} else if (TimeManager.this.overviewInterval < (evt.getEnd() - evt.getStart()) * MIN_OVERVIEW_INTERVAL) {
				TimeManager.this.overviewInterval = (evt.getEnd() - evt.getStart()) * MIN_OVERVIEW_INTERVAL;
				GWT.log("MIN_OVERVIEW_INTERVAL:"+overviewInterval);
			}
			*/
    ...
    public long getIntervalBeginFixed() {
		return intervalBeginFixed;
	}

	public void setIntervalBeginFixed(long intervalBeginFixed) {
		this.intervalBeginFixed = intervalBeginFixed;
	}

	public long getIntervalEndFixed() {
		return intervalEndFixed;
	}

	public void setIntervalEndFixed(long intervalEndFixed) {
		this.intervalEndFixed = intervalEndFixed;
	}

   public boolean getIsOverviewTimeFixed() {
		return isOverviewTimeFixed;
	}

	public void setIsOverviewTimeFixed(boolean isOverviewTimeFixed) {
		this.isOverviewTimeFixed = isOverviewTimeFixed;
	}
}
